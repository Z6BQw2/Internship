...
__global__ void V4_sans_2(__nv_bfloat16* Q, __nv_bfloat16* K, __nv_bfloat16* V, __nv_bfloat16* out, 
    ...
    for (int j = 0; j < seq_len / BLOCK_SIZE; ++j) {
        ...
        for (int p = 0; p < d_model / BLOCK_SIZE - 1; p++){
            ...
            sK[next_buf_idx][ty][tx] = K[(j * BLOCK_SIZE + tx) * d_model + (p + 1) * BLOCK_SIZE + ty];
            __syncthreads();
