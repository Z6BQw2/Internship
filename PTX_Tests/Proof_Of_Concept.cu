...
// ===================================================================
// KERNEL 1: BASELINE STANDARD (AVEC SHARED MEMORY)
// ===================================================================
...
__global__ void Frag_swapped(const __nv_bfloat16* A, const __nv_bfloat16* B, float* out) {
    ...
    // Réduction parallèle intra-warp (au sein des sous-groupes de 4 threads)
    ...
    if(threadIdx.x % 4 == 0){
        int row_group = threadIdx.x / 4; // row_group = 0..7
        
        // La première ligne gérée par ce groupe est `row_group`
        int row_idx_A = row_group;
        // La deuxième ligne gérée par ce groupe est `row_group + 8`
...
int main() {
    ...
    // --- Calcul de la référence CPU ---
    std::vector<float> ref_out;
    cpu_reference(h_A, h_B, ref_out);

    // --- Lancement et Benchmark ---
    dim3 gridDim(1, 1, 1);
