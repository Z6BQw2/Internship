...
__global__ void flash_attention_kernel(__nv_bfloat16* Q, __nv_bfloat16* K, __nv_bfloat16* V, __nv_bfloat16* out, 
    ...
    // Me demandez pas pourquoi, mais mon code ralentit si je supprime ces deux pointeurs. Probablement une histoire de memory alignment ou un truc du genre. Mystère mystère...
    ...
    for (int j = 0; j < seq_len / BLOCK_SIZE; ++j) {
        ...
        for (int p = 0; p < d_model / BLOCK_SIZE; ++p) {
            ...
            wmma::load_matrix_sync(k_frag, &sK[p % 2][0][0], BLOCK_SIZE);
